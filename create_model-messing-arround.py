...
def create_model(all_documents_file, relevance_file,query_file):
    ...
    X = rv[ ["avg_cos_title","cosine_title","cosine_title_w2v","common_title","avg_cos_body","cosine_body","cosine_body_w2v","common_body","max_query_idf","max_pos_query_idf","norm_query_idf","len_query_idf","prob_query_idf","max_title_idf","max_pos_title_idf", "norm_title_idf","len_title_idf", "prob_title_idf","max_body_idf", "max_pos_body_idf", "norm_body_idf", "len_body_idf", "prob_body_idf" ]]
    print(X)
    print(X.shape)
    ...
    # clf = RandomForestClassifier(class_weight=[{1: 1, 2: 1, 3: 1, 4: 1}] ).fit(X_train, y_train)
    ...
    # clf = RandomForestClassifier(class_weight=[{0: 0.1, 1: 1}, {0: 0.1, 1: 0.75}, {0: 0.1, 1: 0.5}, {0: 0.1, 1: 0.25}] ).fit(X_train, y_train)
    # clf = RandomForestClassifier(class_weight=[{0: 1, 1: 0.75, 2: 0.5, 3: 0.25, 4: 0.01}]).fit(X_train, y_train)
