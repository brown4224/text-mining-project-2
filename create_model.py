...
def create_model(all_documents_file, relevance_file,query_file):
    ...
    relevance_with_values = documents


    relevance = pd.read_json(relevance_file)[["query_num", "position", "id"]]
    Y = multi_label(relevance, len(documents))
    ...
    # relevance_with_values["test"] = vectorizer.transform(relevance_with_values["title"])
    X = vectorizer.transform(relevance_with_values["body"])
